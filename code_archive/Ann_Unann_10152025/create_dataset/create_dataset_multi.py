...
for idx in range(SEQ.shape[0]):
    ...
    if NAME[idx].split('.')[0] in orthologs and sys.argv[1] != "all":
        continue
    #print(idx, NAME[idx], CHROM[idx], STRAND[idx], TX_START[idx], TX_END[idx], JN_START[idx])
    X, A0, A1= create_datapoints(SEQ[idx], STRAND[idx], TX_START[idx], TX_END[idx], JN_START[idx])

    for i, y in enumerate(A0[0]):
        # continue if there are no spliced sites in region
        if (np.sum(A0[0][i][:,1])) < 1 or (np.sum(A0[0][i][:,2])) < 1:
            continue
        ...
        Y = np.concatenate([A0[0][i],np.expand_dims(A1[0][i],1)], axis=1)
        h5f2.create_dataset('Y' + str(ctr), data=Y)

        # coordinates
        if STRAND[idx] == '+':
            h5f2.create_dataset('Z' + str(ctr), data=np.array([CHROM[idx], str(int(TX_START[idx])-(CL_max//2)+SL*i-1), str(int(TX_START[idx])+SL*(i+1)+CL_max//2-1), STRAND[idx]]))
        elif STRAND[idx] == '-':
            h5f2.create_dataset('Z' + str(ctr), data=np.array([CHROM[idx], str(int(TX_END[idx])+(CL_max//2)-SL*i), str(int(TX_END[idx])-SL*i-SL-CL_max//2), STRAND[idx]]))
        else:
            print("error")
            exit()
