...
from models import SimpleNetModified_DA_SSE
...
demats = [mat[..., ::-1] for mat in demats]
...
for i in range(len(nets)):
    for j in range(i+1, len(nets)):
        ...
        matchlist.append(np.argwhere(mat > CORR_THRESH).astype(str) + np.array(["_"+str(i), "_"+str(j)], dtype=object)[None, :])
        matchscores.append(mat[mat > CORR_THRESH])
...
output_dir = "./train_parallel_SSE/figures"
...
for cc_i, cc in enumerate(nx.connected_components(g)):
    ...
    for row_i, node in enumerate(cc):
        ...
        donor = demats[matind][0, motifind]
        acceptor = demats[matind][1, motifind]  
    ...
    # --- Save figure ---
    pdf_path = os.path.join(output_dir, f"motif_thresh{CORR_THRESH}_cc{cc_i+1}.pdf")
...
for cc in list(nx.connected_components(g)):
    if len(cc)>=7:  # select only motifs appearing in at least 2 replicates
        for i in cc:
...
pdf_path = os.path.join(output_dir, f"motifs_7replicates_{CORR_THRESH}.pdf")
