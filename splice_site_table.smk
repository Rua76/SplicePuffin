...
if False:
    rule splice_table_test:
        ...
        run:
            ...
            # Get expressed genes (Brain, Heart, Testis, Liver)
            exp = pd.read_csv(input.exp, sep='\t', header=0, index_col=0)
            expr = {}
            for tissue in ["Brain", "Heart", "Testis", "Liver"]:  # Updated tissue order
                tissue_exp = exp.loc[:, exp.columns.str.contains(tissue)]
                for gene, count in tissue_exp.iterrows():
                    expr.setdefault(gene, []).append(count.mean())
            ...
            for snp in intersection:
                if snp.fields[2] == "gene":
                    # Extract fields - new order: Brain(-4), Heart(-3), Testis(-2), Liver(-1)
                    b_val = float(snp.fields[-4])  # Brain value
                    h_val = float(snp.fields[-3])  # Heart value
                    t_val = float(snp.fields[-2])  # Testis value 
                    l_val = float(snp.fields[-1])  # Liver value
                    ...
                    # Get TPM values for this gene (order: Brain, Heart, Testis, Liver)
                    tpm = expr.get(gene, [0.0]*4)  # Default to zeros if gene not found
                    
                    # Apply TPM thresholds - new order matches tissue order
                    if tpm[0] < 2.5: b_val = -3  # Brain
                    if tpm[1] < 2.5: h_val = -3  # Heart
                    if tpm[2] < 2.5: t_val = -3  # Testis (replaces Kidney)
                    if tpm[3] < 2.5: l_val = -3  # Liver
                    
                    # Create entry (same format as splice_table)
                    entry = "%s:%s,%s,%s,%s" % (ss_id, b_val, h_val,t_val, l_val)  # Note order matches template
                    
                    # Add to gene dictionary
                    key = "%s,%s,%s,%s,%s" % (snp.chrom, gene, snp.start, snp.end, ss_strand)
                    gene_dict.setdefault(key, []).append(entry)
            
            # Write output (same format as splice_table)
            with open(output[0], 'w') as fout:
                for gene, snps in gene_dict.items():
                    parts = gene.split(',')
                    fout.write("%s\t0\t%s\t%s\t%s\t%s\t%s\n" % (
                        parts[1], parts[0], parts[-1], parts[2], parts[3], ';'.join(snps) + ';'
                    ))